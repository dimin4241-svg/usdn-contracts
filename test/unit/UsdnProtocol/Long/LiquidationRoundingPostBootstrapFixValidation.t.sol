...
/// @notice Green-side regression for the strongest fully post-bootstrap source
/// witness. The source floors still differ by one wei, but the patched aggregate
/// reconciliation must prevent that dust from reaching the Rebalancer invariant.
contract TestLiquidationRoundingPostBootstrapFixAccounting is PostBootstrapFixFixture {
    ...
    function test_A_sameOrdinaryOnlyWitnessNowLiquidatesSuccessfully() public {
        ...
        assertEq(ticks[0].totalPositions, 1, "tick A count");
        assertEq(ticks[1].totalPositions, 1, "tick B count");
    ...
    function test_B_sameSourceFloorsStillMissExactlyOneWeiButStateIsReconciled() public {
        ...
        uint256 valueA =
            12_675_520_241_288_472_878 * (price - 1_684_916_553_638_498_409_790) / price;
        uint256 valueB =
            12_034_709_889_783_951_966 * (price - 1_668_152_187_831_301_125_432) / price;

        assertEq(valueA, 126_802_320_177_930_074, "tick A source floor");
        assertEq(valueB, 238_935_382_547_897_344, "tick B source floor");
        assertEq(uint256(pnl.tempLongBalance) - valueA - valueB, 1, "same source must still expose one-wei rounding gap");
...
/// @notice Green-side user-impact regression. A real pending withdrawal is added
/// to the same fully post-bootstrap state. After the fixed dedicated liquidation
/// succeeds, the first permissionless withdrawal validation should complete
/// directly instead of being forced into two one-tick recovery transactions.
contract TestLiquidationRoundingPostBootstrapFixImpact is PostBootstrapFixFixture {
    ...
    function setUp() public {
        ...
        // Initiate at the current pre-crash price. Recompute finalPrice after the
        // real pending action exists so the test does not assume vault accounting
        // has no effect on liquidation math.
